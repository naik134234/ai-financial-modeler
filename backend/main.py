...
async def export_to_pdf(job_id: str):
    """
    Export a completed model to PDF format
    
    Args:
        job_id: Job ID of the completed model
    ...
    try:
        ...
        pdf_path = pdf_exporter.export_to_pdf(file_path, job)
        ...
        return FileResponse(
            pdf_path,
            media_type="application/pdf",
            filename=f"{job.get('company_name', 'model')}_{job_id}.pdf"
        )
...
@app.get("/api/export/{job_id}/pptx")
async def export_to_powerpoint(job_id: str):
    """
    Export a completed model to PowerPoint format
    ...
    try:
        ...
        # Generate PowerPoint from Excel file
        pptx_path = pptx_exporter.export_to_pptx(file_path, job)
        ...
        return FileResponse(
            pptx_path,
            media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation",
            filename=f"{job.get('company_name', 'model')}_{job_id}.pptx"
        )
